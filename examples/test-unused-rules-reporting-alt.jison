...
parser.main = function () {
    ...
    // `undefined` value via the injected `$$ = undefined` action code, 
    // while any rule action which does not assign a value to `$$` implicitly
    // will get its value set to `undefined` due to the preset of `$$ = undefined`
    // in the parser core where the 'reduce' action is handled, hence
    // the 'expected output' IS NOT `"εεaε"`, NEITHER `"εεεε"` but `"εεundefinedε"`! 
    //
