...
WORKBOOK_STRING_PREFIX              \'[{UNICODE_LETTER_RANGE},0-9,\.,_, ]+\'[!]
SOURCE                              \[[{UNICODE_LETTER_RANGE},0-9,\.,_, ]+\]
...
//        }
    ...
    | accept_direct_numeric_value
        {
            rv = $accept_direct_numeric_value;
            ...
            $$ = coerceValueToNumeric(rv);
        }

    | accept_direct_string_value
        {
            rv = $accept_direct_string_value;
        }

    | accept_direct_boolean_value
        {
            rv = $accept_direct_boolean_value;
        }
    ...
    | RANGEREF_PARSE_MODE any_range_reference
        {
             ...
             */
            if ($any_range_reference instanceof BasicRangeRefRepresentation) {
                $$ = $any_range_reference;
            } else {
    ...
    | VALUE_PARSE_MODE accept_direct_numeric_value
        {
            /*
             * TODO:
             *
    ...
    | VALUE_PARSE_MODE accept_direct_string_value
        {
            /*
             * TODO:
             *
    ...
    | VALUE_PARSE_MODE accept_direct_boolean_value
        {
            /*
             * TODO:
             *
 ...
 */
    : math_expression[first] BOOLEAN_AND_OPERATOR math_expression[last]
        {
            // Excel: `=AND("T",1)` gives `#VALUE!`; we do not emulate that behaviour here!
            $$ = !!($first && $last);
        }

    | math_expression[first] BOOLEAN_OR_OPERATOR math_expression[last]
        {
            // Excel: `=OR("T",1)` gives `#VALUE!`; we do not emulate that behaviour here!
            $$ = !!($first || $last);
        }
// -----------------------------------------------------------------------------------------------------
    | math_expression[first] NOT_EQUAL math_expression[last]
        {
    ...
    | math_expression[first] NOT_IDENTICAL math_expression[last]
        {
            // WARNING: see the Excel note below at IS_EQUAL: we don't really have a type-agnostic compare available!
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
    ...
    | math_expression[first] ALMOST_EQUAL math_expression[last]
        {
    ...
    | math_expression[first] IS_IDENTICAL math_expression[last]
        {
            // WARNING: see the Excel note below at IS_EQUAL: we don't really have a type-agnostic compare available!
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
    ...
    | math_expression[first] GREATER_OR_EQUAL math_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
    ...
    | math_expression[first] LESS_OR_EQUAL math_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            // Excel: `=FALSE>1.0e300` gives TRUE: we do not emulate that behaviour here.
    ...
    | math_expression[first] '>'[greater_than] math_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            // Excel: `=FALSE>1.0e300` gives TRUE: we do not emulate that behaviour here.
    ...
    | math_expression[first] '<'[less_than] math_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            // Excel: `=FALSE>1.0e300` gives TRUE: we do not emulate that behaviour here.
    ...
    | math_expression[first] '='[equals] math_expression[last]
        {
    ...
    | math_expression[first] IS_EQUAL math_expression[last]
        {
            /*
 ...
 */
    | math_expression[first] STRING_CONCATENATION_OPERATOR math_expression[last]
        {
            $first = coerceValueToString($first);
            $last = coerceValueToString($last);
            $$ = '' + $first + $last;
        }
 ...
 */
    | math_expression[first] '+'[add] math_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
    ...
    | math_expression[first] '-'[subtract] math_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            // Excel: `=FALSE>1.0e300` gives TRUE: we do not emulate that behaviour here.
 ...
 */
    | math_expression[first] '*'[multiply] math_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
    ...
    | math_expression[first] '/'[divide] math_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            $$ = ($first / $last);
    ...
    | math_expression[first] MODULO_OPERATOR math_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            $$ = ($first % $last);
 ...
 * >     = 2**3**2 ~ 2**(3**2) = 512
 */
    | math_expression[first] '^'[power] math_expression[last]
        {
            $first = coerceValueToNumeric($first);
            $last = coerceValueToNumeric($last);
            $$ = Math.pow($first, $last);
    ...
    | SQRT_OPERATOR math_expression
        {
            $math_expression = coerceValueToNumeric($math_expression);
            $$ = Math.sqrt($math_expression);
        }

    | math_expression SQUARE_OPERATOR
        {
            $math_expression = coerceValueToNumeric($math_expression);
            $$ = Math.pow($math_expression, 2);
        }

    | math_expression CUBE_OPERATOR
        {
            $math_expression = coerceValueToNumeric($math_expression);
            $$ = Math.pow($math_expression, 3);
        }

    | math_expression '?'[TOPS20_help_request]
        {
    ...
    : any_range_reference[first] ARRAY_CONCATENATION_OPERATOR any_range_reference[last]
        {
            ...
            if (!($first instanceof parser.RangeRefRepresentation)) {
                range = new parser.RangeRefRepresentation($first, $last);
            } else {
                range = $first;
                range.addRef($last);
            }
            parser.updateRangeLocationInfo(range, $first.locInfo, $first.consumeCount);
            parser.updateRangeLocationInfo(range, $last.locInfo, $last.consumeCount);
...
ambiguous_any_range_reference
    ...
    | any_range_reference                       /* a fragmented range, a cell reference or a regular range reference */
    ...
    | cell_reference
        {
            ...
            range = new parser.RangeRefRepresentation($cell_reference);
            parser.updateRangeLocationInfo(range, $cell_reference.locInfo, $cell_reference.consumeCount);
            $$ = range;
        }
    ...
    : range_reference
        {
            ...
            if (!($range_reference instanceof parser.RangeRefRepresentation)) {
                range = new parser.RangeRefRepresentation($range_reference);
            } else {
...
parser.getArrayValueOfCellRangeSpec = function (rangeSpec) {
    ...
    for (var k = 0, len = rangeSpec.length; k < len; k++) {
        var spec = rangeSpec[k];
        if (!(spec instanceof BasicRangeRefRepresentation)) {
            var cellRef = spec.col + spec.row * MAX_GRID_COL;
...
var FKA_COMMA = 1004;
var FKA_FIXED_ROW_OR_COLUMN_MARKER = 1000;
var FKA_RANGE_MARKER = 1001;
var FKW_ADD = 1;
var FKW_ALMOST_EQUAL = 2;
var FKW_ARRAY_CONCATENATION_OPERATOR = 3;
var FKW_BOOLEAN_AND_OPERATOR = 4;
var FKW_BOOLEAN_NOT_OPERATOR = 5;
var FKW_BOOLEAN_OR_OPERATOR = 6;
var FKW_CUBE_OPERATOR = 7;
var FKW_DATA_MARKER = 8;
var FKW_DEGREES_OPERATOR = 9;
var FKW_DIVIDE = 10;
var FKW_DOT = 11;
var FKW_EQUAL = 12;
var FKW_GREATER_OR_EQUAL = 13; 
var FKW_GREATER_THAN = 14;
var FKW_IS_IDENTICAL = 15;
var FKW_LESS_OR_EQUAL = 16;
var FKW_LESS_THAN = 17;
var FKW_MODULO_OPERATOR = 18;
var FKW_MULTIPLY = 19;
var FKW_NOT_EQUAL = 20;
var FKW_NOT_IDENTICAL = 21;
var FKW_POWER = 22;
var FKW_PROMILAGE_OPERATOR = 23;
var FKW_SQRT_OPERATOR = 24;
var FKW_SQUARE_OPERATOR = 25;
var FKW_STRING_CONCATENATION_OPERATOR = 26; 
var FKW_SUBTRACT = 27;
var FKW_VALUE = 28;
var FNA_1_2_FRACTION = 1;
var FNA_1_3_FRACTION = 9;
var FNA_1_4_FRACTION = 10;
var FNA_1_8_FRACTION = 11;
var FNA_2_3_FRACTION = 12;
var FNA_3_4_FRACTION = 13;
var FNA_3_8_FRACTION = 14;
var FNA_5_8_FRACTION = 15;
var FNA_7_8_FRACTION = 16;
var FNA_CONSTANT_E = 1;
var FNA_CONSTANT_EPSILON = 2;
var FNA_CONSTANT_INFINITY = 3;
var FNA_CONSTANT_LN10 = 4;
var FNA_CONSTANT_LN2 = 5;
var FNA_CONSTANT_LOG2 = 6;
var FNA_CONSTANT_PI = 7;
var FNA_CONSTANT_SYMBOL = 8;
var FPM_CELLREF = 1;
var FPM_FORMULA = 2;
var FPM_IMMEDIATE_VALUE = 3;
var FPM_RANGEREF = 4;
var FSC_CELL_LABEL = 0x0020;
var FSC_CELL_VALUE_PRESET  = 0x0010;
var FSC_COLUMN_LABEL = 0x0008;
var FSC_CURRENCY = 0x0004;
var FSC_FUNCTION = 0x0002;
var FSC_JSON_FILTER_FUNCTION = 0x0040;
var FSC_PREDEFINED_CONSTANT = 0x0001;
var FSC_PREDEFINED_CONSTANT = 0x0001;
var FSC_ROW_LABEL = 0x0080;
var FT_BOOLEAN = 0x2000;
var FT_FORMULA = 0x8000;
var FT_NUMBER = 0x1000;
var FT_STRING = 0x4000;
var FU_ANY = 0x20000;
var FU_BOOLEAN = 0x80000;
var FU_DERIVED = 0x10000;
var FU_STRING = 0x40000;
var OPCODE_MASK_FOR_FT_VALUE_TYPE = 0xF000;
var VQ_DEVELOPER_ONLY = 1;
var VQ_PRODUCTION_READY = 2;

var treatNonNumericsAsZero = true;

var Visyond = {
    Functions: {
    }
};
var vUserPreferences = {
    quality: VQ_DEVELOPER_ONLY
};

// class/constructor for the error value which is carried out of any Excel function;
// think of it as a over-sophisticated NaN of sorts... sigh.
//
// Instances of this MUST be treated as constants (i.e. non-modifiable); that way you
// allow yourself to pass simple references to them everywhere AND enable yourself
// to use quick checks such as `val instanceof ErrorSignal` to see if a given result value
// happens to be an 'Excel-style' error value.
//
// Special methods such as `toString()` may be overridden in this class to provide
// simplified display/printing (I'm thinking of GetAsHumanReadable() here...)

function ErrorSignal(errorValue) {
  this.value = errorValue;
}
// This is for anyone who wants to print us as a string:
ErrorSignal.prototype.toString = function () {
  return this.value;
};
// This is for anyone who hopes to treat us like a Number:
// in calculations, we are best represented by a straight NaN so that
// at least *some kind of error* makes it out the other side
// of whatever calculus you're going to subject us to!
ErrorSignal.prototype.valueOf = function() {
  return NaN;
};
// NOTE/WARNING: depending on the needs of the app/system, e.g. getAsHumanReadable()
// and friends, it MAY be beneficial to us to also override/supply the .toFixed, .toScientific
// etc. Number object member methods in case those are used somewhere for displaying
// calculus result values.
ErrorSignal.prototype.toPrecision = function() {
  return this.value; // which is our #ERROR string, we don't care about the number of digits you want to see!
};
...
// istanceof ErrorSignal
Visyond.Functions.ERROR = {
    nil: new ErrorSignal('#NULL!'),
    div0: new ErrorSignal('#DIV/0!'),
    value: new ErrorSignal('#VALUE?'),
    ref: new ErrorSignal('#REF!'),
    name: new ErrorSignal('#NAME?'),
    num: new ErrorSignal('#NUM!'),
    na: new ErrorSignal('#N/A'),
    error: new ErrorSignal('#ERROR!'),
    data: new ErrorSignal('#GETTING_DATA'),
    unsupported: new ErrorSignal('#UNSUPPORTED!'),
    ext_ref: new ErrorSignal('#EXTERNAL_SOURCE!')
};


function coerceValueToNumeric(cellValue) {
    // Now we handle the case of how the parser behaves when one of the precedent
    // cells is NaN: by default a string is interpreted as zero
    // User can decide whether to change this using the settings pane.

    // NOTE:
    // WARNING: we don't know if this value originated from a function or a direct cell value:
    // it can come from anywhere. We coerce it anyway, so this changes behaviour compared to
    // before (when we made a difference between values coming from formulas and values that don't.)
    //
    // For example, =CONCAT('A', 'B') should always produce a string 'AB'. This is where we screw up that
    // assumption!
    //
    // Also, =TAN(0.5*PI) should always be allowed to output #NAN (and NOT have it 'reset' to output zero(0) because that would subtly impact dependent cells)
    // Also, =LOG10(0) should always be allowed to output -INF (and NOT have it 'reset' to output zero(0) because that would make it LOG10(1) equivalent which is wrong)
    //
    // Conclusion: we always 'transport' NaN, +/-Inf through; anything else gets coerced
    // and the Lord have mercy on the poor souls who feed strings into numeric expressions,
    // because we ain't savin' the buggers!

    // if we're trying to coerce a cellref or rangeref, it's (regrettably) a two-stage process.
    // Thought the idea would not need this, but alas, the whole coerce business here mandates it. :-(
    //
    if (cellValue instanceof ErrorSignal) {
        return cellValue;
    }

    if (cellValue instanceof parser.CellRefRepresentation) {
        cellValue = cellValue.valueOf();
    }

    if (typeof cellValue === 'number' || typeof cellValue === 'boolean') {
        // transport all numeric and boolean values through as is, including +/-Inf and NaN!
    } else if (cellValue == null || cellValue === '') { // null is not enough to cover empty string
        //console.log("empty string or other 'empty/zero' representation, equivalent to === undefined || === null || === '' || === 0");
        cellValue = 0; // an empty string or null is always zero for calculation purposes
    } else {
        // NOTE: Excel coerces strings to numbers in most math operations; we do the same.
        // TODO: for extra strict checking of spreadsheets we should also have a mode where
        // we do not allow auto-coercion like that so that those places where strings are
        // turned into numbers become visible and can be flagged by NaN errors.
        // (The 'clean' way to coerce in Excel is to use the INT() function et al.)

        // THIS SHOULD APPLY TO ALL NON NUMERIC, NOT ONLY ERRORS
        // if (cellValue instanceof ErrorSignal) {
        // if the coercion didn't produce a proper numeric value, then we will either report it as an error
        // or treat it as a fake zero(0):
        if (treatNonNumericsAsZero) {
            cellValue = 0;
        } else {
            cellValue = Visyond.Functions.ERROR.na;
        }
        //}
    }
...
// This is a duplicate of `coerceValuetoNumeric()` but instead outputs a STRING value.
// Done as a duplicate for performance: we call this one where we need it and don't bother
// `coerceValueToNumeric()` with our particular string antics.
function coerceValueToString(cellValue) {
    // See `coerceValueToNumeric()` internal comments for why we do all this: these two functions
    // MUST match in this behaviour!
    if (cellValue instanceof ErrorSignal) {
        return cellValue;
    }
...
function get_predefined_formula_constants() {
    var predefined_formula_constants =
    [
        {
            category: FSC_PREDEFINED_CONSTANT | FSC_FUNCTION,
            name: "PI",
            alts: [ "\u03c0" /* π */ ],
            quality: VQ_PRODUCTION_READY,
            notation: FNA_CONSTANT_PI,
            value: Math.PI
        },
        {
            category: FSC_PREDEFINED_CONSTANT | FSC_FUNCTION,
        ...
        },
        {
            ...
            alts: [ "\u221e" /* ∞ */ ],
            quality: VQ_PRODUCTION_READY,
            notation: FNA_CONSTANT_INFINITY,
            value: Number.POSITIVE_INFINITY
        },
        {
            category: FSC_PREDEFINED_CONSTANT | FSC_FUNCTION,
            name: "E",
            quality: VQ_DEVELOPER_ONLY,         // WARNING: see issue Visyond/visyond#780
            notation: FNA_CONSTANT_E,
            value: Math.E
        },
        {
            category: FSC_PREDEFINED_CONSTANT | FSC_FUNCTION,
        ...
        },
        {
            ...
            name: "TRUE",
            quality: VQ_PRODUCTION_READY,
            notation: FNA_CONSTANT_SYMBOL,
            attributes: FKW_VALUE | FT_BOOLEAN | FU_BOOLEAN,
            description: "Produces the logical value TRUE.",
            value: true
        },
        {
            category: FSC_PREDEFINED_CONSTANT | FSC_FUNCTION,
            ...
            notation: FNA_CONSTANT_SYMBOL,
            attributes: FKW_VALUE | FT_BOOLEAN | FU_BOOLEAN,
            description: "Produces the logical value FALSE.",
            value: false
        },
        {
            category: FSC_PREDEFINED_CONSTANT,
            name: "\u2153",                            /* ⅓ */
            quality: VQ_DEVELOPER_ONLY,
            notation: FNA_1_3_FRACTION,
            attributes: FKW_VALUE,
            value: (1.0 / 3.0)
        },
        {
            category: FSC_PREDEFINED_CONSTANT,
        ...
        },
        {
            ...
            name: "\u00be",                            /* ¾ */
            quality: VQ_DEVELOPER_ONLY,
            notation: FNA_3_4_FRACTION,
            attributes: FKW_VALUE,
            value: (3.0 / 4.0)
        }
    ];
    return predefined_formula_constants;
}

function get_predefined_formula_currencies() {
    return [];
}
function get_predefined_formula_functions() {
    return [];
}
...
var old_pre_parse = parser.pre_parse;
parser.pre_parse = function (yy) {
    ...
    parser.lexer.options.patch_tokenstream = function () {
    };
    parser.lexer.options.patch_resulttype = function () {
    };

    //console.log('exec old preparse? ', typeof old_pre_parse);
    if (old_pre_parse) {
        old_pre_parse.call(this, yy);
    }
};
...
parser.main = function () {
    ...
    function test() {
        const formulas_and_expectations =  [
            '=' + basenum + '+2*(3-5--+--+6!)-7/-8%',                      Visyond.Functions.ERROR.na,
            '=' + basenum + '+2*E/100^PI^2+4+5',                              9 + basenum, /* this bets on JS floating point calculations discarding the small difference with this integer value... */
            '=' + basenum + '+(2+3*1+1+1+1+1)+5+6+7+8+9/9',                     36 + basenum, // with error recovery and all it gives you a value...
        ...
        for (var i = 0, len = formulas_and_expectations.length; i < len; i += 2) {
            ...
            print("'" + formula + "' ==> ", rv, "\n");
            assert.equal(rv && rv.value, expectation);
        }
