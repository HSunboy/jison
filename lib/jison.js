...
if (typeof console !== 'undefined' && console.log) {
    ...
    Jison.print = function console_log(...args) {
    ...
    Jison.print = function puts_print(...args) {
        puts(args.join(' '));
    };
...
generator.constructor = function Jison_Generator(grammar, optionalLexerSection, options) {
    ...
    // and standardize it:
    let preliminary_options = mkStdOptions(options);
...
generator.processGrammar = function processGrammarDef(grammar) {
    ...
    if (grammar.imports) {
        ...
        if (symbols_import) {
            predefined_symbols = helpers.extractSymbolTableFromFile(symbols_import.path, 'symbols_');
...
generator.buildProductions = function buildProductions(bnf, productions, nonterminals, symbols, operators, predefined_symbols, descriptions) {
    ...
    function collectLiteralTokensInProduction(handle) {
        ...
        try {
            if (devDebug) {
                Jison.print('\ncollectLiteralTokensInProduction: ', this.symbol, ':', JSON.stringify(handle, null, 2), ' @ options: ', this);
            }
            ...
            if (handle.constructor === Array) {
                ...
                for (let i = 0; i < rhs.length; i++) {
                    ...
                    let rhs_i = sym.match(new XRegExp(`\\[${ID_REGEX_BASE}\\]$`));
                    if (rhs_i) {
    ...
    function splitStringIntoSymbols(rhs) {
        ...
        while (pos1 >= 0 || pos2 >= 0) {
            ...
            rhs = rhs.substr(pos + 1);
            let alias = rhs.match(new XRegExp(`^\\[${ID_REGEX_BASE}\\]`));
            if (alias) {
                ls += alias[0];
    ...
    function buildProduction(handle) {
        ...
        if (devDebug) {
            Jison.print('\nbuildProduction: ', this.symbol, ':', JSON.stringify(handle, null, 2));
        }
        ...
        if (handle.constructor === Array) {
            ...
            for (let i = 0; i < rhs.length; i++) {
                // check for aliased names, e.g., id[alias] and strip them
                let rhs_i = rhs[i].match(new XRegExp(`\\[${ID_REGEX_BASE}\\]$`));
                if (rhs_i) {
...
generator.buildProductionActions = function buildProductionActions() {
    ...
    */
    ...
    let moduleInclude = preprocessActionCode(this.moduleInclude)
        .replace(new XRegExp(`#(${ID_REGEX_BASE})#`, 'g'), function (_, sym) {
    ...
    while (gen_hash !== prev_gen_hash) {
        ...
        let parameters = 'yy, yytext, yyleng, yylineno, yyloc, yystate /* action[1] */, yysp, yyrulelength, yyvstack, yylstack, yystack, yysstack';
...
generator.warn = function jison_warn(...args) {
    Jison.print.call(null, 'WARNING: ' + (args.join('') || '???'));
};
...
lookaheadMixin.followSets = function followSets() {
    ...
    while (cont) {
        ...
        productions.forEach(function Follow_prod_forEach(production, k) {
            if (devDebug > 3) {
                Jison.print('Symbol/Follows: ', 'round:' + count, 'prod:' + k, ':', production.symbol, ' --> ', nonterminals[production.symbol].follows.join(', '));
            }
...
lrGeneratorMixin.canonicalCollection = function canonicalCollection() {
    ...
    if (devDebug > 0) {
        Jison.print('canonicalCollection: ', states.has);
    }
                ...
                if (action) {
                    ...
                    if (sol.bydefault) {
                        ...
                        if (!self.conflict_fixing_round && self.options.hasPartialLrUpgradeOnConflict) {
                            ...
                            if (devDebug > 4) {
                                Jison.print('Registering conflict: ', {
...
generatorMixin.createParser = function createParser() {
    ...
    let sourcecode = rmCommonWS`
        ...
        yy__parser;
    `;
    let p = code_exec(sourcecode, function generated_code_exec_wrapper_jison(sourcecode) {
...
generatorMixin.createLexer = function createLexer(lexerSpec, input, tokens, options) {
    ...
    // lexer_options = ...
    let lexer = new Lexer(lexerSpec, input, tokens, options);
...
//_handle_error_end_of_section:                   // this concludes the error recovery / no error recovery code section choice above
    ...
    if (sharedState_yy.yydebug === false) {
        ...
        sharedState_yy.yydebug = yydebug;
    }
        ...
        this.yyError = function yyError(str, ...args) {
            if (yydebug) {
                yydebug('yyerror: ', {
                    message: str,
                    args: arguments,
                    symbol, state, newState, recoveringSymbolShiftCounter, action
                });
            }
...
//_handle_error_with_recovery:                    // run this code when the grammar includes error recovery rules
    ...
    if (this.yyRecovering) {
        this.yyRecovering = function yyRecovering() {
            if (yydebug) yydebug('yyrecovering: ', { symbol, state, newState, recoveringSymbolShiftCounter, action });
            return recoveringSymbolShiftCounter;
        };
    }

    if (this.yyErrOk) {
        this.yyErrOk = function yyErrOk() {
            if (yydebug) yydebug('yyerrok: ', { symbol, state, newState, recoveringSymbolShiftCounter, action });
            recoveringSymbolShiftCounter = 0;
    ...
    if (this.yyClearIn) {
        this.yyClearIn = function yyClearIn() {
            if (yydebug) yydebug('yyclearin: ', { symbol, newState, recoveringSymbolShiftCounter, action, preErrorSymbol });
            if (symbol === TERROR) {
                symbol = 0;
                yytext = null;
                yyleng = 0;
...
//_handle_error_end_of_section:                   // this concludes the error recovery / no error recovery code section choice above
    ...
    this.constructParseErrorInfo = function parser_constructParseErrorInfo(msg, ex, expected, recoverable) {
        ...
        //
        let i;
        for (i = sp; pei.state_stack[i] != null; i++) {
            // empty
        }
        // reserve at least ERROR_RECOVERY_TOKEN_DISCARD_COUNT+1 extra NULL slots below the base for further error processing
        i += ERROR_RECOVERY_TOKEN_DISCARD_COUNT + 1;
        pei.base_pointer = i;
        pei.info_stack_pointer = i;
    ...
    // clone some parts of the (possibly enhanced!) errorInfo object
    // to give them some persistence, i.e. take a snapshot of the stacks, etc.
...
//_lexer_with_token_stack_end:
    ...
    let yyval = {
        $: true,
        _$: undefined,
        // since 0.7.0: OBSOLETED as the performAction() calling interface has changed a bit and we don't want to risk cyclic references in the parser.
        // 
        // yy: sharedState_yy
    };
    let p;
            ...
            if (this.defaultActions[state]) {
                ...
                if (yydebug) {
                    yydebug('after FETCH/LEX: ', {
                        ...
                        state, newState, recoveringSymbolShiftCounter, action
                    });
                }
...
//_handle_error_with_recovery:                // run this code when the grammar includes error recovery rules
                ...
                if (!action) {
                    ...
                    if (error_rule_depth < 0) {
                        ASSERT(recoveringSymbolShiftCounter > 0, 'Line 7828');
                        recoveringErrorInfo.info_stack_pointer = esp;
                    ...
                    r = this.performAction.call(yyval, yy, yytext, yyleng, yylineno, yyloc, combineState, sp - 1, yyrulelen, vstack, lstack, stack, sstack);
                    ...
                    ASSERT(recoveringErrorInfo, 'Line 7986');
                    ASSERT(symbol === TERROR, 'Line 7987');
                    ASSERT(!action, 'Line 7988');
                    ...
                    for (;;) {
                        ...
                        if (this.defaultActions[state]) {
                            ...
                            if (!action) {
                                ...
                                ASSERT(recoveringErrorInfo, 'Line 8030');
                        ...
                        if (yydebug) {
                            ...
                            (action === 1 ?
                                ...
                                action, newState, recoveringSymbolShiftCounter, symbol
                            });
                        }
                        ...
                        case 1:
                            ...
                            if (!preErrorSymbol) { // normal execution / no error
                                ...
                                if (recoveringSymbolShiftCounter > 0) {
                                    ...
                                    if (yydebug) yydebug('... SHIFT:error rule matching: ', { recoveringSymbolShiftCounter, symbol });
                                }
                            } else {
                                // error just occurred, resume old lookahead f/ before error, *unless* that drops us straight back into error mode:
                                ASSERT(recoveringSymbolShiftCounter > 0, 'Line 8134');
                                symbol = preErrorSymbol;
                                preErrorSymbol = 0;
                                if (yydebug) yydebug('... SHIFT:error recovery: ', { recoveringSymbolShiftCounter, symbol });
                                // read action for current state and first input
                                t = (table[newState] && table[newState][symbol]) || NO_ACTION;
                                if (!t[0] || symbol === TERROR) {
                                    ...
                                    if (yydebug) yydebug('... SHIFT:error recovery: re-application of old symbol doesn\\'t work: instead, we\\'re moving forward now. ', { recoveringSymbolShiftCounter, symbol });
                                    symbol = 0;
                                }
                            }
                            ...
                            ASSERT(preErrorSymbol === 0, 'Line 8165');
                            if (recoveringSymbolShiftCounter === 0) {
                                break;
                            }
                            continue;
                        ...
                        case 2:
                            ...
                            if (yydebug) {
                                yydebug('~~~ REDUCE: ', {
                                    pop_size: yyrulelen,
                                    newState, recoveringSymbolShiftCounter, symbol
                                });
                            }

                            r = this.performAction.call(yyval, yy, yytext, yyleng, yylineno, yyloc, newState, sp - 1, yyrulelen, vstack, lstack, stack, sstack);
                            ...
                            // don't overwrite the \`symbol\` variable: use a local var to speed things up:
                            {
                                ...
                                sstack[sp] = newState;
                                ++sp;
                                if (yydebug) yydebug('REDUCED: ', { newState, recoveringSymbolShiftCounter, symbol });
                            }
                            continue;
...
//_handle_error_end_of_section:                  // this concludes the error recovery / no error recovery code section choice above
            ...
            case 1:
                ...
                ASSERT(preErrorSymbol === 0, 'Line 8408');         // normal execution / no error
                ASSERT(recoveringSymbolShiftCounter === 0, 'Line 8409');             // normal execution / no error
            ...
            // reduce:
            case 2:
                ASSERT(preErrorSymbol === 0, 'Line 8420');         // normal execution / no error
                ASSERT(recoveringSymbolShiftCounter === 0, 'Line 8421');             // normal execution / no error
...
const LR0Generator = Jison.LR0Generator = lr0.construct();
        ...
        these_states.forEach(function union_states_forEach(state, i) {
            ...
            if (state.reductions.length && treat_me) {
                state.reductions.forEach(function union_reduction_forEach(item) {
                    ...
                    if (devDebug > 2) {
                        Jison.print('not-yet-unioned item', {
                            handle,
                            item,
                            follows,
                            goes: state.goes,
                    ...
                    state.goes[handle].forEach(function reduction_goes_forEach(symbol) {
                        newg.nonterminals[symbol].follows.forEach(function goes_follows_forEach(symbol) {
                            ...
                            if (!follows[terminal]) {
                                ...
                                if (devDebug > 2) {
                                    Jison.print('adding to FOLLOW set (union)', {
                                        terminal,
                                        nonterminal: symbol,
...
const LALRGenerator = Jison.LALRGenerator = lalr.construct();
 ...
 * SLR Parser
 */
const SLRGenerator = Jison.SLRGenerator = lrLookaheadGenerator.construct({
    type: 'SLR(1)',
...
const LR1Generator = Jison.LR1Generator = lr1.construct();
...
const LLGenerator = Jison.LLGenerator = ll.construct();
...
function Parser(g, l, options) {
    let gen = Jison.Generator(g, l, options);
