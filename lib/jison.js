...
generator.processGrammar = function processGrammarDef(grammar) {
    ...
    if (grammar.imports) {
        ...
        if (symbols_import) {
            let filepath = path.resolve(symbols_import.path);
            ...
            try {
                ...
                if (devDebug) {
                    Jison.print('', '%import symbols JSON fail: ', ex);
                }
                try {
                    let m = /[\r\n]\s*symbols_:\s*(\{[\s\S]*?\}),\s*[\r\n]/.exec(source);
                    if (m && m[1]) {
                    ...
                    if (devDebug) {
                        Jison.print('', '%import symbols JISON output fail: ', ex);
                    }
...
//_handle_error_with_recovery:                // run this code when the grammar includes error recovery rules
                ...
                if (!action) {
                    ...
                    ASSERT(recoveringErrorInfo, 'Line 7834');
                    ASSERT(symbol === TERROR, 'Line 7835');
                    ASSERT(!action, 'Line 7836');
                    ...
                    for (;;) {
                        ...
                        if (this.defaultActions[state]) {
                            ...
                            if (!action) {
                                ...
                                ASSERT(recoveringErrorInfo, 'Line 7878');
                        ...
                        case 1:
                            ...
                            ASSERT(preErrorSymbol === 0, 'Line 8013');
                            if (recovering === 0) {
                                break;
                            }
                            continue;
...
//_handle_error_end_of_section:                  // this concludes the error recovery / no error recovery code section choice above
            ...
            case 1:
                ...
                ASSERT(preErrorSymbol === 0, 'Line 8249');         // normal execution / no error
                ASSERT(recovering === 0, 'Line 8250');             // normal execution / no error
            ...
            // reduce:
            case 2:
                ASSERT(preErrorSymbol === 0, 'Line 8261');         // normal execution / no error
                ASSERT(recovering === 0, 'Line 8262');             // normal execution / no error
