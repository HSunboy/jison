...
let mkIdentifier  = helpers.mkIdentifier;
let code_exec  = helpers.exec;
...
generator.buildProductions = function buildProductions(bnf, productions, nonterminals, symbols, operators, predefined_symbols, descriptions) {
    ...
    function collectLiteralTokensInProduction(handle) {
        ...
        try {
            ...
            console.error(ex, '\ncollectLiteralTokensInProduction: ', symbol, ':', JSON.stringify(handle, null, 2), ' @ options: ', this, { i, sym, rhs });
            throw ex;
        }
    }
...
lrGeneratorMixin.generateModule_ = function generateModule_() {
    ...
    let exportSourceCode = this.options.exportSourceCode;
...
let api_set = (new Function('', 'return { ' + define_parser_APIs_1 + ' };'))();
...
//_handle_error_with_recovery:                // run this code when the grammar includes error recovery rules
                ...
                if (!action) {
                    ...
                    ASSERT(recoveringErrorInfo, 'line 1145');
                    ASSERT(symbol === TERROR, 'line 1146');
                    ASSERT(!action, 'line 1147');
...
let lr0 = generator.beget(lookaheadMixin, generatorMixin, lrGeneratorMixin, {
    type: 'LR(0)',
...
let LR0Generator = Jison.LR0Generator = lr0.construct();

/*
 * Simple LALR(1)
 */

let lalr = generator.beget(lookaheadMixin, generatorMixin, lrGeneratorMixin, {
    type: 'LALR(1)',
...
let LALRGenerator = Jison.LALRGenerator = lalr.construct();
 ...
 * Define base type
 */
let lrLookaheadGenerator = generator.beget(lookaheadMixin, generatorMixin, lrGeneratorMixin, {
 ...
 * SLR Parser
 */
let SLRGenerator = Jison.SLRGenerator = lrLookaheadGenerator.construct({
    type: 'SLR(1)',
 ...
 * LR(1) Parser
 */
let lr1 = lrLookaheadGenerator.beget({
    type: 'Canonical LR(1)',
...
let LR1Generator = Jison.LR1Generator = lr1.construct();

/*
 * LL Parser
 */
let ll = generator.beget(lookaheadMixin, generatorMixin, lrGeneratorMixin, {
    type: 'LL(1)',
...
let LLGenerator = Jison.LLGenerator = ll.construct();
