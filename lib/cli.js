...
function getCommandlineOptions() {
    const defaults = Jison.defaultJisonOptions;
    let opts = nomnom
        .script('jison')
        ...
        .options({
            ...
            defaultActionMode: {
                ...
                transform: function (val) {
                    ...
                    let v = ('' + val).split(',').map(function cvt_modes(mode, idx) {
                        mode = mode.trim();
                        switch (mode) {
                        case 'false':
                        case '0':
...
function cliMain(opts) {
    ...
    function processInputFile() {
        ...
        try {
            ...
            opts.lexfile = path.resolve(opts.lexfile);

            if (opts.lexfile) {
                lex = fs.readFileSync(opts.lexfile, 'utf8');
            }
...
//      let settings = Jison.mkStdOptions(opts);
    ...
    let generator = new Jison.Generator(grammar, optionalLexSection, opts);
