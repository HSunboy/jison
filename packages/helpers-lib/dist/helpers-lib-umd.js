(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('fs'), require('path'), require('assert')) :
    typeof define === 'function' && define.amd ? define(['fs', 'path', 'assert'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global["jison-helpers-lib"] = factory(global.fs, global.path, global.assert));
})(this, (function (fs, path, assert) { 'use strict';
    ...
    function rmCommonWS(strings) {
      ...
      for (var i = 0, len = arguments.length <= 1 ? 0 : arguments.length - 1; i < len; i++) {
        rv.push(src[i].join('\n'));
        rv.push(i + 1 < 1 || arguments.length <= i + 1 ? undefined : arguments[i + 1]);
      }
    ...
    function dumpSourceToFile(sourcecode, errname, err_id, options, ex) {
      var dumpfile;
      try {
        var dumpPaths = [options.outfile ? path__namespace.dirname(options.outfile) : null, options.inputPath, process.cwd()];
        var dumpName = path__namespace.basename(options.inputFilename || options.moduleName || (options.outfile ? path__namespace.dirname(options.outfile) : null) || options.defaultModuleName || errname).replace(/\.[a-z]{1,5}$/i, '') // remove extension .y, .yacc, .jison, ...whatever
        ...
        for (var i = 0, l = dumpPaths.length; i < l; i++) {
          ...
          try {
            dumpfile = path__namespace.normalize(dumpPaths[i] + '/' + dumpName);
            fs__namespace.writeFileSync(dumpfile, sourcecode, 'utf8');
