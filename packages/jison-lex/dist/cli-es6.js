...
function generateSourcePrelude(position) {
    ...
    assert$1(typeof position === 'object');
...
function extractSymbolTableFromFile(filepath, sectionName) {
    ...
    assert$1(predefined_symbols ? !import_error : import_error);
    if (import_error) {
    ...
    assert$1(predefined_symbols);
    assert$1(typeof predefined_symbols === 'object');
...
function setupFileBasedTestRig(__DIRNAME__ /* __dirname */, testset, __NAME__, options) {
    ...
    function assertOutputMatchesReference(ist, soll, msg) {
        ...
        assert$1.strictEqual(typeof ist, 'string');
        assert$1.strictEqual(typeof soll, 'string');
        ...
        assert$1.strictEqual(ist, soll, msg);
    }
...
function prepareRules(dict, actions, caseHelper, tokens, startConditions, grammarSpec) {
    ...
    assert$1.equal(typeof grammarSpec.options['case-insensitive'], 'undefined');
    ...
    for (let i = 0; i < rules.length; i++) {
        ...
        // - fault
        // 
        assert$1(('srcCode' in rule) || rule.fault);
        assert$1(!rule.fault || rule.fault instanceof Error);  // when there's a 'fault' reported for this rule, it must be an Error object.
...
function RegExpLexer(dict, input, tokens, build_options, yy) {
    ...
    let lexer = test_me(null, null, null, function onTestFailure_1(ex) {
        ...
        assert$1(grammarSpec.options);
        assert$1(grammarSpec.options.xregexp !== undefined);
        ...
        if (!test_me(function tweakResult_1() {
            ...
            if (!test_me(function tweakResult_2() {
                ...
                assert$1(Array.isArray(grammarSpec.rules));
                ...
                if (!test_me(function tweakResult_3() {
                    ...
                    try {
                        ...
                        for (let i = rules.length - 1; i >= 0; i--) {
                            ...
                            let rule = rules[i];
                            assert$1(rule != null);
                            assert$1.equal(typeof rule, 'object');
                            assert$1('rule' in rule);
                            assert$1('srcCode' in rule);
                            rule.srcCode = '{ /* nada */ }';
...
function generateModuleBody(grammarSpec) {
    ...
    if (grammarSpec.rules.length > 0 || grammarSpec.__in_rules_failure_analysis_mode__) {
        ...
        assert$1.strictEqual(grammarSpec.regular_rule_count, 0);
        assert$1.strictEqual(grammarSpec.simple_rule_count, 0);
...
assert$1(RegExpLexer);
assert$1(RegExpLexer.defaultJisonLexOptions);
assert$1(typeof RegExpLexer.mkStdOptions === 'function');
...
function cliMain(opts) {
    ...
    function processInputFile() {
        const original_cwd = process$1.cwd();
        ...
        try {
            ...
            mkdirp$1(path.dirname(opts.outfile));
    ...
    function readin(cb) {
        const stdin = process$1.openStdin();
        let data = '';
