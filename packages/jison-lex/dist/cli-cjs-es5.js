...
function bp(s){let rv=[];let p=s.pop;let r=s.rule;for(let i=0,l=p.length;i<l;i++){rv.push([p[i],r[i]]);}return rv;}// helper: reconstruct the defaultActions[] table
...
}}}return tokenset;},productions_:bp({pop:u([55,s,[56,5],57,58,58,s,[59,20],s,[60,10,1],s,[69,12],s,[70,5],71,71,s,[72,7],s,[73,7],74,74,75,76,76,s,[77,5],78,78,s,[79,18],80,81,81,82,82,83,83,84,85,85,s,[86,3],s,[87,4],88,88,89,89,s,[90,3],s,[91,3],s,[92,4],93,93]),rule:u([4,3,3,2,2,0,0,2,0,3,2,3,2,c,[4,3],1,2,c,[6,3],1,3,3,6,5,5,3,s,[1,10],2,2,4,2,c,[41,4],s,[2,4],0,2,4,c,[53,4],0,s,[4,3],s,[3,3],s,[2,7],0,4,c,[46,3],c,[68,3],2,c,[44,3],c,[62,3],c,[24,7],c,[12,3],s,[1,7],c,[17,3],c,[9,7],c,[8,3],c,[13,8],c,[35,5],c,[17,5],2])}),performAction:function parser__PerformAction(yyloc,yystate/* action[1] */,yysp,yyvstack,yylstack){/* this == yyval */ // the JS engine itself can go and remove these statements when `yy` turns out to be unused in any action code!
...
this._$=yyparser.yyMergeLocationInfo(yysp-1,yysp);// END of default action (generated by JISON mode classic/merge :: 2/2,VT,VA,VU,-,LT,LA,-,-)
{let start_marker=yyvstack[yysp-1].trim();let marker_msg=start_marker?' or similar, such as '+start_marker:'';yyparser.yyError(rmCommonWS$1`
...
{let start_marker=yyvstack[yysp-2].trim();let marker_msg=start_marker?' or similar, such as '+start_marker:'';let end_marker_msg=marker_msg.replace(/\{/g,'}');yyparser.yyError(rmCommonWS$1`
...
this._$=yyparser.yyMergeLocationInfo(yysp-2,yysp);// END of default action (generated by JISON mode classic/merge :: 3/3,VT,VA,VU,-,LT,LA,-,-)
{let start_marker=yyvstack[yysp-1].trim();yyparser.yyError(rmCommonWS$1`
        There's very probably a problem with this '%{...%}' lexer setup action code section.
...
console.error("*** error! marker:",start_marker);if(start_marker.indexOf('{')>=0){let marker_msg=start_marker?' or similar, such as '+start_marker:'';yyparser.yyError(rmCommonWS$1`
            The '%{...%}' lexer setup action code section MUST have its action
            block start marker (\`%{\`${marker_msg}) positioned
            at the start of a line to be accepted: *indented* action code blocks
            (such as this one) are always related to an immediately preceding lexer spec item,
...
this.$=yyvstack[yysp-2];this._$=yyparser.yyMergeLocationInfo(yysp-2,yysp);// END of default action (generated by JISON mode classic/merge :: 3/3,VT,VA,-,-,LT,LA,-,-)
{// rewrite + accept star '*' as name + check if we allow empty list?
let lst=yyvstack[yysp-1].map(function(el){return el[0];});yyparser.yyError(rmCommonWS$1`
...
if(yy.__options_flags__&OPTION_DOES_NOT_ACCEPT_MULTIPLE_OPTIONS){yyparser.yyError(rmCommonWS$1`
        ...
        `);}if(yy.__options_flags__&OPTION_DOES_NOT_ACCEPT_COMMA_SEPARATED_OPTIONS){let optlist=yyvstack[yysp-2].map(function(opt){return opt[0];});optlist.push(yyvstack[yysp][0]);yyparser.yyError(rmCommonWS$1`
...
this.$=yyvstack[yysp-1];this._$=yyparser.yyMergeLocationInfo(yysp-1,yysp);// END of default action (generated by JISON mode classic/merge :: 2/2,VT,VA,-,-,LT,LA,-,-)
{let with_value_msg=' (with optional value assignment)';if(yy.__options_flags__&OPTION_DOES_NOT_ACCEPT_VALUE){with_value_msg='';}yyparser.yyError(rmCommonWS$1`
...
if(!isLegalIdentifierInput$1(yyvstack[yysp])){let with_value_msg=' (with optional value assignment)';if(yy.__options_flags__&OPTION_DOES_NOT_ACCEPT_VALUE){with_value_msg='';}yyparser.yyError(rmCommonWS$1`
...
this._$=yylstack[yysp];// END of default action (generated by JISON mode classic/merge :: 1/1,VT,VA,VU,-,LT,LA,-,-)
{// validate that this is legal input under the given circumstances, i.e. parser context:
if(!(yy.__options_flags__&OPTION_EXPECTS_ONLY_IDENTIFIER_NAMES)||yy.__options_flags__&OPTION_ALSO_ACCEPTS_STAR_AS_IDENTIFIER_NAME){this.$=yyvstack[yysp];}else{let with_value_msg=' (with optional value assignment)';if(yy.__options_flags__&OPTION_DOES_NOT_ACCEPT_VALUE){with_value_msg='';}yyparser.yyError(rmCommonWS$1`
            Expected a valid name/argument${with_value_msg} in a ${yy.__options_category_description__} statement.
            Entries (names) must look like regular programming language
            identifiers, with the addition that option names MAY contain
            '-' dashes, e.g. 'example-option-1'
...
this._$=yyparser.yyMergeLocationInfo(yysp-1,yysp);// END of default action (generated by JISON mode classic/merge :: 2/2,VT,VA,VU,-,LT,LA,-,-)
{let srcCode=trimActionCode$1(yyvstack[yysp]);if(srcCode){let rv=checkActionBlock$1(srcCode,yylstack[yysp],yy);if(rv){yyparser.yyError(rmCommonWS$1`
...
let fileContent=fs__default['default'].readFileSync(path,{encoding:'utf-8'});let srcCode=trimActionCode$1(fileContent);if(srcCode){let rv=checkActionBlock$1(srcCode,this._$,yy);if(rv){yyparser.yyError(rmCommonWS$1`
...
x=i.toString(16);if(x.length>=1&&i<=0xFFFF){c='0000'+x;return'\\u'+c.substr(c.length-4);}return'\\u{'+x+'}';}return String.fromCharCode(i);}// Helper collection for `bitarray2set()`: we have expanded all these cached `\\p{NAME}` regex sets when creating
...
let Pcodes_bitarray_cache={};let Pcodes_bitarray_cache_test_order=[];// Helper collection for `bitarray2set()` for minifying special cases of result sets which can be represented by
// a single regex 'escape', e.g. `\d` for digits 0-9.
let EscCode_bitarray_output_refs;// now initialize the EscCodes_... table above:
init_EscCode_lookup_table();function init_EscCode_lookup_table(){let s,bitarr,set2esc={},esc2bitarr={};// patch global lookup tables for the time being, while we calculate their *real* content in this function:
...
bitarr=[];set2bitarray(bitarr,WORDCHAR_SETSTR);s=bitarray2set(bitarr);esc2bitarr['w']=bitarr;set2esc[s]='w';// set2esc['^' + s] = 'W';
Pcodes_bitarray_cache['\\w']=bitarr;EscCode_bitarray_output_refs={esc2bitarr:esc2bitarr,set2esc:set2esc};updatePcodesBitarrayCacheTestOrder();}function updatePcodesBitarrayCacheTestOrder(opts){let t=new Array(UNICODE_BASE_PLANE_MAX_CP+1);let l={};let user_has_xregexp=opts&&opts.options&&opts.options.xregexp;let i,j,k,ba;// mark every character with which regex pcodes they are part of:
for(k in Pcodes_bitarray_cache){ba=Pcodes_bitarray_cache[k];if(!user_has_xregexp&&k.indexOf('\\p{')>=0){continue;}let cnt=0;for(i=0;i<=UNICODE_BASE_PLANE_MAX_CP;i++){if(ba[i]){cnt++;if(!t[i]){t[i]=[k];}else{t[i].push(k);}}}l[k]=cnt;}// now dig out the unique ones: only need one per pcode.
//
...
let w=Infinity;var rv;ba=Pcodes_bitarray_cache[k];for(i=0;i<=UNICODE_BASE_PLANE_MAX_CP;i++){if(ba[i]){let tl=t[i].length;if(tl>1&&tl<w){assert__default['default'](l[k]>0);rv=[i,k];w=tl;}}}if(rv){done[k]=true;lut.push(rv);}}}// order from large set to small set so that small sets don't gobble
...
lut.sort(function(a,b){let k1=a[1];let k2=b[1];let ld=l[k2]-l[k1];if(ld){return ld;}// and for same-size sets, order from high to low unique identifier.
...
let pex=c1+c2;let ba4p=Pcodes_bitarray_cache[pex];if(!ba4p){// expand escape:
...
let xs=''+xr;// remove the wrapping `/.../` to get at the (possibly *combined* series of) `[...]` sets inside:
...
c1='\u0008';break;}}let v1=eval_escaped_code(c1);// propagate deferred exceptions = error reports.
...
c2=s[0];}else{c2=c2[0];}let v2=eval_escaped_code(c2);// propagate deferred exceptions = error reports.
...
// range then.
if(set_is_inverted){for(let i=0;i<=UNICODE_BASE_PLANE_MAX_CP;i++){if(!bitarr[i]){bitarr_orig[i]=true;}}}}return false;}// convert a simple bitarray back into a regex set `[...]` content:
...
let rv=[];let i,j,cnt,lut,tn,tspec,match,pcode,ba4pcode,l2;let bitarr_is_cloned=false;let l_orig=l;if(output_inverted_variant){// generate the inverted set, hence all unmarked slots are part of the output range:
...
rv.push(i2c(i));if(j-1>i){rv.push((j-2>i?'-':'')+i2c(j-1));}i=j;}}assert__default['default'](rv.length);let s=rv.join('');assert__default['default'](s);// Check if the set is better represented by one of the regex escapes:
let esc4s=EscCode_bitarray_output_refs.set2esc[s];if(esc4s){// When we hit a special case like this, it is always the shortest notation, hence wins on the spot!
...
function reduceRegexToSetBitArray(s,name,opts){let orig=s;// propagate deferred exceptions = error reports.
if(s instanceof Error){return s;}let l=new Array(UNICODE_BASE_PLANE_MAX_CP+1);let internal_state=0;let derr;while(s.length){let c1=s.match(CHR_RE);if(!c1){// cope with illegal escape sequences too!
return new Error('illegal escape sequence at start of regex part: "'+s+'" of regex "'+orig+'"');}c1=c1[0];s=s.substr(c1.length);switch(c1){case'[':// this is starting a set within the regex: scan until end of set!
var set_content=[];while(s.length){let inner=s.match(SET_PART_RE);if(!inner){inner=s.match(CHR_RE);if(!inner){// cope with illegal escape sequences too!
return new Error('illegal escape sequence at start of regex part: '+s+'" of regex "'+orig+'"');}inner=inner[0];if(inner===']')break;}else{inner=inner[0];}set_content.push(inner);s=s.substr(inner.length);}// ensure that we hit the terminating ']':
var c2=s.match(CHR_RE);if(!c2){// cope with illegal escape sequences too!
return new Error('regex set expression is broken in regex: "'+orig+'" --> "'+s+'"');}c2=c2[0];if(c2!==']'){return new Error('regex set expression is broken in regex: '+orig);}s=s.substr(c2.length);var se=set_content.join('');if(!internal_state){derr=set2bitarray(l,se,opts);// propagate deferred exceptions = error reports.
...
// inside a regex set:
try{assert__default['default'](s);assert__default['default'](!(s instanceof Error));let re=new XRegExp__default['default']('['+s+']');re.test(s[0]);// One thing is apparently *not* caught by the RegExp compile action above: `[a[b]c]`
...
let s1=bitarray2set(bitarr,false,true);// and when the regex set turns out to match a single pcode/escape, then
// use that one as-is:
...
return s1;}s1='['+s1+']';// Now try to produce a minimum regex from the *inverted* bitarray via negation:
// Because we look at a negated bitset, there's no use looking for matches with
// special cases here.
let s2=bitarray2set(bitarr,true,true);if(s2[0]==='^'){s2=s2.substr(1);if(s2.match(SET_IS_SINGLE_PCODE_RE)){// When we hit a special case like this, it is always the shortest notation, hence wins on the spot!
...
let s3=bitarray2set(bitarr,false,false);// and when the regex set turns out to match a single pcode/escape, then
// use that one as-is:
...
return s3;}s3='['+s3+']';// Now try to produce a minimum regex from the *inverted* bitarray via negation:
// Because we look at a negated bitset, there's no use looking for matches with
// special cases here.
let s4=bitarray2set(bitarr,true,false);if(s4[0]==='^'){s4=s4.substr(1);if(s4.match(SET_IS_SINGLE_PCODE_RE)){// When we hit a special case like this, it is always the shortest notation, hence wins on the spot!
...
const rmCommonWS$2=helpers.rmCommonWS;const mkIdentifier$2=helpers.mkIdentifier;const code_exec=helpers.exec;const version='0.6.2-220';// require('./package.json').version;
...
//
// This is the base XRegExp ID regex used in many places; this should match the ID macro definition in the EBNF/BNF parser et al as well!
const ID_REGEX_BASE$1='[\\p{Alphabetic}_][\\p{Alphabetic}_\\p{Number}]*';// see also ./lib/cli.js
/**
@public
@nocollapse
...
/** @public */function mkStdOptions()/*...args*/{let h=Object.prototype.hasOwnProperty;let opts={};let args=[].concat.apply([],arguments);// clone defaults, so we do not modify those constants?
if(args[0]!=='NODEFAULT'){args.unshift(defaultJisonLexOptions);}else{args.shift();}for(let i=0,len=args.length;i<len;i++){let o=args[i];if(!o)continue;// clone input (while camel-casing the options), so we do not modify those either.
let o2={};for(var p in o){if(typeof o[p]!=='undefined'&&h.call(o,p)){o2[mkIdentifier$2(p)]=o[p];}}// now clean them options up:
...
let exportSourceCode=options.exportSourceCode;if(!exportSourceCode||typeof exportSourceCode!=='object'){exportSourceCode={enabled:!!exportSourceCode};}else if(typeof exportSourceCode.enabled!=='boolean'){exportSourceCode.enabled=true;}options.exportSourceCode=exportSourceCode;}// Autodetect if the input lexer spec is in JSON or JISON
...
function autodetectAndConvertToJSONformat(lexerSpec,options){let chk_l=null;let ex1,err;if(typeof lexerSpec==='string'){if(options.json){try{chk_l=JSON5__default['default'].parse(lexerSpec);// When JSON5-based parsing of the lexer spec succeeds, this implies the lexer spec is specified in `JSON mode`
...
function prepareRules(dict,actions,caseHelper,tokens,startConditions,opts){let m,i,k,rule,action,conditions;let active_conditions;assert__default['default'](Array.isArray(dict.rules));let rules=dict.rules.slice(0);// shallow copy of the rules array as we MAY modify it in here!
let newRules=[];let macros={};let regular_rule_count=0;let simple_rule_count=0;// Assure all options are camelCased:
...
return str;}let routingCode=['switch(yyrulenumber) {'];for(i=0;i<rules.length;i++){rule=rules[i].slice(0);// shallow copy: do not modify input rules
...
function reduceRegex(s,name,opts,expandAllMacrosInSet_cb,expandAllMacrosElsewhere_cb){let orig=s;function errinfo(){if(name){return'macro [['+name+']]';}return'regex [['+orig+']]';}// propagate deferred exceptions = error reports.
if(s instanceof Error){return s;}let c1,c2;let rv=[];let derr;let se;while(s.length){c1=s.match(CHR_RE$1);if(!c1){// cope with illegal escape sequences too!
return new Error(errinfo()+': illegal escape sequence at start of regex part: '+s);}c1=c1[0];s=s.substr(c1.length);switch(c1){case'[':// this is starting a set within the regex: scan until end of set!
var set_content=[];var l=new Array(UNICODE_BASE_PLANE_MAX_CP$1+1);while(s.length){let inner=s.match(SET_PART_RE$1);if(!inner){inner=s.match(CHR_RE$1);if(!inner){// cope with illegal escape sequences too!
return new Error(errinfo()+': illegal escape sequence at start of regex part: '+s);}inner=inner[0];if(inner===']')break;}else{inner=inner[0];}set_content.push(inner);s=s.substr(inner.length);}// ensure that we hit the terminating ']':
c2=s.match(CHR_RE$1);if(!c2){// cope with illegal escape sequences too!
return new Error(errinfo()+': regex set expression is broken: "'+s+'"');}c2=c2[0];if(c2!==']'){return new Error(errinfo()+': regex set expression is broken: apparently unterminated');}s=s.substr(c2.length);se=set_content.join('');// expand any macros in here:
...
// it in a regex; that way we can easily validate whether macro X is fit to be used
// inside a regex set:
try{let re;re=new XRegExp__default['default'](s);re.test(s[0]);}catch(ex){// make sure we produce a regex expression which will fail badly when it is used
// in actual code:
...
function prepareMacros(dict_macros,opts){let macros={};// expand a `{NAME}` macro which exists inside a `[...]` set:
function expandMacroInSet(i){let k,a,m;if(!macros[i]){m=dict_macros[i];if(m.indexOf('{')>=0){// set up our own record so we can detect definition loops:
...
if(m===false){return new Error('Macro name "'+i+'" has an illegal, looping, definition, i.e. it\'s definition references itself, either directly or indirectly, via other macros.');}}return m;}function expandMacroElsewhere(i){let m;if(macros[i].elsewhere==null){m=dict_macros[i];// set up our own record so we can detect definition loops:
...
if(m===false){return new Error('Macro name "'+i+'" has an illegal, looping, definition, i.e. it\'s definition references itself, either directly or indirectly, via other macros.');}}return m;}function expandAllMacrosInSet(s){let i,x;// process *all* the macros inside [...] set:
if(s.indexOf('{')>=0){for(i in macros){if(macros.hasOwnProperty(i)){let a=s.split('{'+i+'}');if(a.length>1){x=expandMacroInSet(i);assert__default['default'](x);if(x instanceof Error){return new Error('failure to expand the macro ['+i+'] in set ['+s+']: '+x.message);}s=a.join(x);}// stop the brute-force expansion attempt when we done 'em all:
if(s.indexOf('{')===-1){break;}}}}return s;}function expandAllMacrosElsewhere(s){let i,x;// When we process the remaining macro occurrences in the regex
...
let a=s.split('{'+i+'}');if(a.length>1){x=expandMacroElsewhere(i);assert__default['default'](x);if(x instanceof Error){return new Error('failure to expand the macro ['+i+'] in regex /'+s+'/: '+x.message);}s=a.join('(?:'+x+')');}// stop the brute-force expansion attempt when we done 'em all:
if(s.indexOf('{')===-1){break;}}}}return s;}let i;if(opts.debug)console.log('\n############## RAW macros: ',dict_macros);// first we create the part of the dictionary which is targeting the use of macros
...
function expandMacros(src,macros,opts){let expansion_count=0;// By the time we call this function `expandMacros` we MUST have expanded and cached all macros already!
// Hence things should be easy in there:
function expandAllMacrosInSet(s){let i,m,x;// process *all* the macros inside [...] set:
if(s.indexOf('{')>=0){for(i in macros){if(macros.hasOwnProperty(i)){m=macros[i];let a=s.split('{'+i+'}');if(a.length>1){x=m.in_set;assert__default['default'](x);if(x instanceof Error){// this turns out to be an macro with 'issues' and it is used, so the 'issues' do matter: bombs away!
...
if(s.indexOf('{')===-1){break;}}}}return s;}function expandAllMacrosElsewhere(s){let i,m,x;// When we process the main macro occurrences in the regex
// every macro used in a lexer rule will become its own capture group.
//
// Meanwhile the cached expansion will expand any submacros into
// *NON*-capturing groups so that the backreference indexes remain as you'ld
...
//
// This is a BREAKING CHANGE from vanilla jison 0.4.15!
if(s.indexOf('{')>=0){for(i in macros){if(macros.hasOwnProperty(i)){m=macros[i];let a=s.split('{'+i+'}');if(a.length>1){// These are all main macro expansions, hence CAPTURING grouping is applied:
...
//
// This is a BREAKING CHANGE from vanilla jison 0.4.15!
let s2=reduceRegex(src,null,opts,expandAllMacrosInSet,expandAllMacrosElsewhere);// propagate deferred exceptions = error reports.
...
if(s2.length<src.length){src=s2;}}return src;}function prepareStartConditions(conditions){let sc;let hash={};for(sc in conditions){if(conditions.hasOwnProperty(sc)){hash[sc]={rules:[],inclusive:!conditions[sc]};}}return hash;}function buildActions(dict,tokens,opts){let actions=[dict.actionInclude||'','var YYSTATE = YY_START;'];let tok;let toks={};let caseHelper=[];// tokens: map/array of token numbers to token names
for(tok in tokens){let idx=parseInt(tok);if(idx&&idx>0){toks[tokens[tok]]=idx;}}let gen=prepareRules(dict,actions,caseHelper,tokens&&toks,opts.conditions,opts);let code=actions.join('\n');'yytext yyleng yylineno yylloc yyerror'.split(' ').forEach(function(yy){code=code.replace(new RegExp('\\b('+yy+')\\b','g'),'yy_.$1');});return{caseHelperInclude:'{\n'+caseHelper.join(',')+'\n}',actions:`function lexer__performAction(yy, yyrulenumber, YY_START) {
            var yy_ = this;
...
return prelude;}const jisonLexerErrorDefinition=generateErrorClass();function generateFakeXRegExpClassSrcCode(){return rmCommonWS$2`
    ...
    `;}/** @constructor */function RegExpLexer(dict,input,tokens,build_options){let opts;let dump=false;function test_me(tweak_cb,description,src_exception,ex_callback){opts=processGrammar(dict,tokens,build_options);opts.__in_rules_failure_analysis_mode__=false;prepExportStructures(opts);assert__default['default'](opts.options);if(tweak_cb){tweak_cb();}let source=generateModuleBody(opts);try{// The generated code will always have the `lexer` variable declared at local scope
...
// const lexer = { bla... };
// ```
let testcode=['// provide a local version for test purposes:',jisonLexerErrorDefinition,'',generateFakeXRegExpClassSrcCode(),'',source,'',rmCommonWS$2`
...
if(src_exception&&description){let msg=description;if(typeof description==='function'){msg=description();}src_exception.message+='\n        ('+msg+')';}// patch the pre and post handlers in there, now that we have some live code to work with:
if(opts.options){let pre=opts.options.pre_lex;let post=opts.options.post_lex;// since JSON cannot encode functions, we'll have to do it manually now:
...
// }
if(ex_callback){ex_callback(ex);}else if(dump){console.log('source code:\n',source);}return false;}}/** @constructor */let lexer=test_me(null,null,null,function(ex){// When we get an exception here, it means some part of the user-specified lexer is botched.
//
...
let rv=false;for(var i=0,len=rulesSpecSize;i<len;i++){var lastEditedRuleSpec;rv=test_me(function(){assert__default['default'](Array.isArray(opts.rules));assert__default['default'](opts.rules.length===rulesSpecSize);// opts.conditions = [];
// opts.rules = [];
// opts.__in_rules_failure_analysis_mode__ = true;
// nuke all rules' actions up to and including rule numero `i`:
for(let j=0;j<=i;j++){// rules, when parsed, have 2 or 3 elements: [conditions, handle, action];
// now we want to edit the *action* part:
let rule=opts.rules[j];assert__default['default'](Array.isArray(rule));assert__default['default'](rule.length===2||rule.length===3);rule.pop();rule.push('{ /* nada */ }');lastEditedRuleSpec=rule;}},function(){return'Your lexer rule "'+lastEditedRuleSpec[0]+'" action code block is botched?';},ex,null);if(rv){break;}}if(!rv){test_me(function(){opts.conditions=[];opts.rules=[];opts.performAction='null';// opts.options = {};
...
/**  @public */function generate(dict,tokens,build_options){let opt=processGrammar(dict,tokens,build_options);return generateFromOpts(opt);}// process the grammar and build final data structures and functions
/**  @public */function processGrammar(dict,tokens,build_options){build_options=build_options||{};let opts={// include the knowledge passed through `build_options` about which lexer
...
*/let performActionCode=String(opt.performAction);let simpleCaseActionClustersCode=String(opt.caseHelperInclude);let rulesCode=generateRegexesInitTableCode(opt);let conditionsCode=cleanupJSON(JSON.stringify(opt.conditions,null,2));code.push(rmCommonWS$2`,
...
return out;}function generateGenericHeaderComment(){let out=rmCommonWS$2`
...
if(!opt.moduleName||!opt.moduleName.match(/^[a-zA-Z_$][a-zA-Z0-9_$\.]*$/)){if(opt.moduleName){let msg='WARNING: The specified moduleName "'+opt.moduleName+'" is illegal (only characters [a-zA-Z0-9_$] and "." dot are accepted); using the default moduleName "lexer" instead.';if(typeof opt.warn_cb==='function'){opt.warn_cb(msg);}else{// do not treat as warning; barf hairball instead so that this oddity gets noticed right away!
throw new Error(msg);}}opt.moduleName='lexer';}prepExportStructures(opt);return opt;}function generateModule(opt){opt=prepareOptions(opt);let modIncSrc=opt.moduleInclude?opt.moduleInclude+';':'';let src=rmCommonWS$2`
    ...
    `;src=stripUnusedLexerCode(src,opt);opt.exportSourceCode.all=src;return src;}function generateAMDModule(opt){opt=prepareOptions(opt);let modIncSrc=opt.moduleInclude?opt.moduleInclude+';':'';let src=rmCommonWS$2`
        ${generateGenericHeaderComment()}
    ...
    `;src=stripUnusedLexerCode(src,opt);opt.exportSourceCode.all=src;return src;}function generateESModule(opt){opt=prepareOptions(opt);let modIncSrc=opt.moduleInclude?opt.moduleInclude+';':'';let src=rmCommonWS$2`
        ${generateGenericHeaderComment()}
    ...
    `;src=stripUnusedLexerCode(src,opt);opt.exportSourceCode.all=src;return src;}function generateCommonJSModule(opt){opt=prepareOptions(opt);let modIncSrc=opt.moduleInclude?opt.moduleInclude+';':'';let src=rmCommonWS$2`
        ${generateGenericHeaderComment()}
...
let outpath=opts.outfile;if(typeof outpath==='string'){if(/[\\\/]$/.test(outpath)||isDirectory(outpath)){opts.outfile=null;outpath=outpath.replace(/[\\\/]$/,'');}else{outpath=path__default['default'].dirname(outpath);}}else{outpath=null;}if(outpath&&outpath.length>0){outpath+='/';}else{outpath='';}// setting output file name and module name based on input file name
// if they aren't specified.
let name=path__default['default'].basename(opts.outfile||opts.file);// get the base name (i.e. the file name without extension)
// i.e. strip off only the extension and keep any other dots in the filename
name=path__default['default'].basename(name,path__default['default'].extname(name));opts.outfile=opts.outfile||outpath+name+'.js';if(!opts.moduleName&&name){opts.moduleName=opts.defaultModuleName=mkIdentifier$3(name);}// Change CWD to the directory where the source grammar resides: this helps us properly
// %include any files mentioned in the grammar with relative paths:
let new_cwd=path__default['default'].dirname(path__default['default'].normalize(opts.file));process.chdir(new_cwd);let lexer=cli.generateLexerString(raw,opts);// and change back to the CWD we started out with:
process.chdir(original_cwd);opts.outfile=path__default['default'].normalize(opts.outfile);mkdirp(path__default['default'].dirname(opts.outfile));fs__default['default'].writeFileSync(opts.outfile,lexer);console.log('JISON-LEX output for module ['+opts.moduleName+'] has been written to file:',opts.outfile);}function readin(cb){const stdin=process.openStdin();let data='';stdin.setEncoding('utf8');stdin.addListener('data',function(chunk){data+=chunk;});stdin.addListener('end',function(){cb(data);});}function processStdin(){readin(function processStdinReadInCallback(raw){console.log(cli.generateLexerString(raw,opts));});}// if an input file wasn't given, assume input on stdin
if(opts.file){processInputFile();}else{processStdin();}}function generateLexerString(lexerSpec,opts){// var settings = RegExpLexer.mkStdOptions(opts);
let predefined_tokens=null;return RegExpLexer.generate(lexerSpec,predefined_tokens,opts);}var cli={main:cliMain,generateLexerString:generateLexerString};if(require.main===module){const opts=getCommandlineOptions();cli.main(opts);}module.exports=cli;
