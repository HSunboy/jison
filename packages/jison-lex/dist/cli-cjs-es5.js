...
throw new Error(msg);}}opt.moduleName='lexer';}prepExportStructures(opt);return opt;}function generateModule(opt){opt=prepareOptions(opt);var modIncSrc=opt.moduleInclude?opt.moduleInclude+';':'';var src=rmCommonWS$2`
    ...
    `;src=stripUnusedLexerCode(src,opt);opt.exportSourceCode.all=src;return src;}function generateAMDModule(opt){opt=prepareOptions(opt);var modIncSrc=opt.moduleInclude?opt.moduleInclude+';':'';var src=rmCommonWS$2`
        ${generateGenericHeaderComment()}
    ...
    `;src=stripUnusedLexerCode(src,opt);opt.exportSourceCode.all=src;return src;}function generateESModule(opt){opt=prepareOptions(opt);var modIncSrc=opt.moduleInclude?opt.moduleInclude+';':'';var src=rmCommonWS$2`
        ${generateGenericHeaderComment()}
    ...
    `;src=stripUnusedLexerCode(src,opt);opt.exportSourceCode.all=src;return src;}function generateCommonJSModule(opt){opt=prepareOptions(opt);var modIncSrc=opt.moduleInclude?opt.moduleInclude+';':'';var src=rmCommonWS$2`
        ${generateGenericHeaderComment()}
...
fs__default['default'].mkdirSync(fp);return true;}catch(e){return false;}}}}return false;}function processInputFile(){var original_cwd=process.cwd();var raw=fs__default['default'].readFileSync(path__default['default'].normalize(opts.file),'utf8');// making best guess at json mode
...
var outpath=opts.outfile;if(typeof outpath==='string'){if(/[\\\/]$/.test(outpath)||isDirectory(outpath)){opts.outfile=null;outpath=outpath.replace(/[\\\/]$/,'');}else{outpath=path__default['default'].dirname(outpath);}}else{outpath=null;}if(outpath&&outpath.length>0){outpath+='/';}else{outpath='';}// setting output file name and module name based on input file name
...
var new_cwd=path__default['default'].dirname(path__default['default'].normalize(opts.file));process.chdir(new_cwd);var lexer=cli.generateLexerString(raw,opts);// and change back to the CWD we started out with:
...
var predefined_tokens=null;return RegExpLexer.generate(lexerSpec,predefined_tokens,opts);}var cli={main:cliMain,generateLexerString:generateLexerString};if(require.main===module){var opts=getCommandlineOptions();cli.main(opts);}module.exports=cli;
