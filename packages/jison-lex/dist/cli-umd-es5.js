...
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('fs'), require('path'), require('@gerhobbelt/nomnom'), require('recast'), require('@babel/core'), require('assert'), require('@gerhobbelt/xregexp'), require('@gerhobbelt/json5')) : typeof define === 'function' && define.amd ? define(['fs', 'path', '@gerhobbelt/nomnom', 'recast', '@babel/core', 'assert', '@gerhobbelt/xregexp', '@gerhobbelt/json5'], factory) : (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global['jison-lex'] = factory(global.fs, global.path$1, global.nomnom, global.recast, global.babel, global.assert$1, global.XRegExp, global.json5));
  ...
  var json5__default = /*#__PURE__*/_interopDefaultLegacy(json5); // Return TRUE if `src` starts with `searchString`. 
  ...
  var lexer = function () {
    ...
    var dquote = helpers.dquote; // unescape a string value which is wrapped in quotes/doublequotes
  ...
  const UNICODE_BASE_PLANE_MAX_CP = 65535; // The expanded regex sets which are equivalent to the given `\\{c}` escapes:
  //
  // `/\s/`:
  ...
  function i2c(i) {
    ...
    if (i < 32 || i > 0xFFF0
    /* Unicode Specials, also in UTF16 */
    || i >= 0xD800 && i <= 0xDFFF
    /* Unicode Supplementary Planes; we're TOAST in JavaScript as we're NOT UTF-16 but UCS-2! */
    || String.fromCharCode(i).match(/[\u2028\u2029]/)
    /* Code compilation via `new Function()` does not like to see these, or rather: treats them as just another form of CRLF, which breaks your generated regex code! */
    ) {
  ...
  var Pcodes_bitarray_cache_test_order = []; // Helper collection for `bitarray2set()` for minifying special cases of result sets which can be represented by 
  ...
  function init_EscCode_lookup_table() {
    var s,
        bitarr,
        set2esc = {},
        esc2bitarr = {}; // patch global lookup tables for the time being, while we calculate their *real* content in this function:

    EscCode_bitarray_output_refs = {
      esc2bitarr: {},
      set2esc: {}
    };
    Pcodes_bitarray_cache_test_order = []; // `/\S':

    bitarr = [];
    set2bitarray(bitarr, '^' + WHITESPACE_SETSTR);
    s = bitarray2set(bitarr);
    esc2bitarr['S'] = bitarr;
    set2esc[s] = 'S'; // set2esc['^' + s] = 's';
  ...
  function updatePcodesBitarrayCacheTestOrder(opts) {
    ...
    var i, j, k, ba; // mark every character with which regex pcodes they are part of:
    ...
    for (j = 0; keys[j]; j++) {
      ...
      if (!done[k]) {
        assert__default['default'](l[k] > 0); // find a minimum span character to mark this one:
  ...
  function set2bitarray(bitarr, s, opts) {
    ...
    function eval_escaped_code(s) {
      var c; // decode escaped code? If none, just take the character as-is
    ...
    if (s && s.length) {
      ...
      while (s.length) {
        ...
        if (!c1) {
          ...
          c1 = c1[0]; // Quick hack for XRegExp escapes inside a regex `[...]` set definition: we *could* try to keep those
          ...
          switch (c1) {
            case '\\p':
              ...
              if (c2) {
                c2 = c2[0];
                s = s.substr(c2.length); // do we have this one cached already?
        ...
        var v1 = eval_escaped_code(c1); // propagate deferred exceptions = error reports.
        ...
        if (s[0] === '-' && s.length >= 2) {
          ...
          var v2 = eval_escaped_code(c2); // propagate deferred exceptions = error reports.
          ...
          v2 = v2.charCodeAt(0);
          s = s.substr(c2.length); // legal ranges go UP, not /DOWN!

          if (v1 <= v2) {
            mark(v1, v2);
          } else {
            console.warn('INVALID CHARACTER RANGE found in regex: ', {
              re: orig,
              start: c1,
              start_n: v1,
              end: c2,
  ...
  function bitarray2set(l, output_inverted_variant, output_minimized) {
    ...
    if (output_inverted_variant) {
      ...
      if (output_minimized) {
        ...
        for (tn = 0; lut[tn]; tn++) {
          tspec = lut[tn]; // check if the uniquely identifying char is in the inverted set:
          ...
          if (!l[tspec[0]]) {
            ...
            if (match && match > pcode.length) {
              rv.push(pcode); // and nuke the bits in the array which match the given pcode:
      ...
      if (output_minimized) {
        ...
        for (tn = 0; lut[tn]; tn++) {
          tspec = lut[tn]; // check if the uniquely identifying char is in the set:
    ...
    var s = rv.join('');
    assert__default['default'](s); // Check if the set is better represented by one of the regex escapes:
  ...
  function reduceRegexToSetBitArray(s, name, opts) {
    var orig = s; // propagate deferred exceptions = error reports.
    ...
    while (s.length) {
      ...
      switch (c1) {
        case '[':
          ...
          if (!internal_state) {
            derr = set2bitarray(l, se, opts); // propagate deferred exceptions = error reports.
        ...
        default:
          ...
          if (!internal_state) {
            derr = set2bitarray(l, c1, opts); // propagate deferred exceptions = error reports.
    ...
    s = bitarray2set(l); // When this result is suitable for use in a set, than we should be able to compile
    ...
    assert__default['default'](s); // propagate deferred exceptions = error reports.
  ...
  function produceOptimizedRegex4Set(bitarr) {
    ...
    s2 = '[' + s2 + ']'; // Then, as some pcode/escapes still happen to deliver a LARGER regex string in the end,
  ...
  const UNICODE_BASE_PLANE_MAX_CP$1 = setmgmt.UNICODE_BASE_PLANE_MAX_CP; // WARNING: this regex MUST match the regex for `ID` in ebnf-parser::bnf.l jison language lexer spec! (`ID = [{ALPHA}]{ALNUM}*`)
  //
  ...
  */
  ...
  const defaultJisonLexOptions = {
    ...
    warn_cb: undefined,
    // function(msg) | true (= use Jison.Print) | false (= throw Exception)
    xregexp: false,
    lexerErrorsAreRecoverable: false,
    flex: false,
    backtrack_lexer: false,
    ranges: false,
    // track position range, i.e. start+end indexes in the input string
    trackPosition: true,
    // track line+column position in the input string
  ...
  /*...args*/
  {
    ...
    var opts = {};
    var args = [].concat.apply([], arguments); // clone defaults, so we do not modify those constants?
    ...
    for (var i = 0, len = args.length; i < len; i++) {
      var o = args[i];
      if (!o) continue; // clone input (while camel-casing the options), so we do not modify those either.
      ...
      delete o2.main; // special check for `moduleName` to ensure we detect the 'default' moduleName entering from the CLI
  ...
  function prepareRules(dict, actions, caseHelper, tokens, startConditions, opts) {
    ...
    var simple_rule_count = 0; // Assure all options are camelCased:
    ...
    routingCode.push('}'); // only inject the big switch/case chunk when there's any `switch` or `default` branch to switch to:
  ...
  function reduceRegex(s, name, opts, expandAllMacrosInSet_cb, expandAllMacrosElsewhere_cb) {
    ...
    while (s.length) {
      ...
      switch (c1) {
        case '[':
          ...
          s = s.substr(c2.length);
          se = set_content.join(''); // expand any macros in here:
        ...
        case '\\p':
          ...
          if (c2) {
            c2 = c2[0];
            s = s.substr(c2.length); // nothing to expand.
    ...
    s = rv.join(''); // When this result is suitable for use in a set, than we should be able to compile
    // it in a regex; that way we can easily validate whether macro X is fit to be used
    // inside a regex set:
  ...
  function prepareMacros(dict_macros, opts) {
    var macros = {}; // expand a `{NAME}` macro which exists inside a `[...]` set:
    ...
    function expandMacroInSet(i) {
      ...
      if (!macros[i]) {
        ...
        var s1; // propagate deferred exceptions = error reports.
    ...
    function expandMacroElsewhere(i) {
      ...
      if (macros[i].elsewhere == null) {
        m = dict_macros[i]; // set up our own record so we can detect definition loops:
    ...
    function expandAllMacrosInSet(s) {
      var i, x; // process *all* the macros inside [...] set:
    ...
    function expandAllMacrosElsewhere(s) {
      var i, x; // When we process the remaining macro occurrences in the regex
  ...
  function expandMacros(src, macros, opts) {
    var expansion_count = 0; // By the time we call this function `expandMacros` we MUST have expanded and cached all macros already!
    // Hence things should be easy in there:

    function expandAllMacrosInSet(s) {
      var i, m, x; // process *all* the macros inside [...] set:
    ...
    function expandAllMacrosElsewhere(s) {
      var i, m, x; // When we process the main macro occurrences in the regex
      // every macro used in a lexer rule will become its own capture group.
      //
      // Meanwhile the cached expansion will expand any submacros into
      // *NON*-capturing groups so that the backreference indexes remain as you'ld
        ...
        for (i in macros) {
          if (macros.hasOwnProperty(i)) {
            ...
            if (a.length > 1) {
              ...
              x = m.elsewhere;
              assert__default['default'](x); // detect definition loop:
  ...
  function buildActions(dict, tokens, opts) {
    ...
    var tok;
    var toks = {};
    var caseHelper = []; // tokens: map/array of token numbers to token names
    ...
    return {
      ...
      rules: gen.rules,
      macros: gen.macros,
      // propagate these for debugging/diagnostic purposes
    ...
    function test_me(tweak_cb, description, src_exception, ex_callback) {
      ...
      try {
        ...
        if (opts.options) {
          ...
          var post = opts.options.post_lex; // since JSON cannot encode functions, we'll have to do it manually now:
      ...
      if (!test_me(function () {
        ...
        if (!test_me(function () {
          ...
          if (!test_me(function () {
            ...
            assert__default['default'](Array.isArray(opts.rules));
            rulesSpecSize = opts.rules.length; // opts.conditions = [];
            ...
            for (var i = 0, len = rulesSpecSize; i < len; i++) {
              ...
              rv = test_me(function () {
                assert__default['default'](Array.isArray(opts.rules));
                assert__default['default'](opts.rules.length === rulesSpecSize); // opts.conditions = [];
                // opts.rules = [];
                // opts.__in_rules_failure_analysis_mode__ = true;
            ...
            if (!rv) {
              test_me(function () {
                opts.conditions = [];
                opts.rules = [];
                opts.performAction = 'null'; // opts.options = {};
...
`)(); // The lexer code stripper, driven by optimization analysis settings and
  ...
  function stripUnusedLexerCode(src, opt) {
    ...
    var new_src = helpers.prettyPrintAST(ast, opt); // inject analysis report now:
  ...
  function generateModuleBody(opt) {
    ...
    function produceOptions(opts) {
      ...
      var do_not_pass = {
        debug: !opts.debug,
        // do not include this item when it is FALSE as there's no debug tracing built into the generated grammar anyway!
        enableDebugLogs: 1,
        json: 1,
        _: 1,
        noMain: 1,
      ...
      var pre = obj.pre_lex;
      var post = obj.post_lex; // since JSON cannot encode functions, we'll have to do it manually at run-time, i.e. later on:
      ...
      js = js.replace(new XRegExp__default['default'](`  "(${ID_REGEX_BASE})": `, 'g'), '  $1: ');
  ...
  function prepareOptions(opt) {
    opt = opt || {}; // check for illegal identifier
  ...
  function generateESModule(opt) {
    opt = prepareOptions(opt);
    var out = [generateGenericHeaderComment(), '', 'var lexer = (function () {', jisonLexerErrorDefinition, '', generateModuleBody(opt), '', opt.moduleInclude ? opt.moduleInclude + ';' : '', '', 'return lexer;', '})();', '', 'function yylex() {', '    return lexer.lex.apply(lexer, arguments);', '}', rmCommonWS$2`
            export {
                lexer,
                yylex as lex
            };
  ...
  function cliMain(opts) {
    ...
    function processInputFile() {
      ...
      if (!opts.moduleName && name) {
        opts.moduleName = opts.defaultModuleName = mkIdentifier$2(name);
      ...
      var lexer = cli.generateLexerString(raw, opts); // and change back to the CWD we started out with:
