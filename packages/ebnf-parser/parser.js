...
case 5:
    ...
    {
    let rv = checkActionBlock(yyvstack[yysp], yylstack[yysp]);
    if (rv) {
        yyparser.yyError(rmCommonWS`
...
case 87:
    ...
    if (yyvstack[yysp]) {
        let rv = checkActionBlock(yyvstack[yysp].action, yylstack[yysp]);
        if (rv) {
            if (!yyvstack[yysp].isArrowAction) {
                ...
                let indentedSrc = rmCommonWS([yyvstack[yysp].action]).split('\n').join('\n    ');
...
case 115:
    ...
    this.$ = {
        action: trimActionCode(yyvstack[yysp]),
        isArrowAction: false
    }
    break;
...
case 116:
    ...
    {
    let src = trimActionCode(yyvstack[yysp], {
...
case 165:       // === NO_ACTION[1] :: ensures that anyone (but us) using this new state will fail dramatically!
