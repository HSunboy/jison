(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('@gerhobbelt/xregexp'), require('fs'), require('path'), require('@gerhobbelt/json5'), require('recast'), require('assert')) :
    typeof define === 'function' && define.amd ? define(['@gerhobbelt/xregexp', 'fs', 'path', '@gerhobbelt/json5', 'recast', 'assert'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global['ebnf-parser'] = factory(global.XRegExp, global.fs, global.path, global.JSON5, global.recast, global.assert));
}(this, (function (XRegExp, fs, path, JSON5, recast, assert$1) { 'use strict';
        ...
        return production.map(function (handle) {
            ...
            if (action) {
                ...
                if (list.first_transformed_term_index) {
                    ...
                    let alias_re = new XRegExp__default['default'](`\\[${ID_REGEX_BASE$1}\\]`);
                    let term_re = new XRegExp__default['default'](`^${ID_REGEX_BASE$1}$`);
                    ...
                    //
                    let nameref_re = new XRegExp__default['default'](`(?:[$@]|##)${ID_REGEX_BASE$1}`, 'g');
    ...
    case 5:
        ...
        {
        let rv = checkActionBlock$1(yyvstack[yysp], yylstack[yysp]);
        if (rv) {
            yyparser.yyError(rmCommonWS$2`
    ...
    case 87:
        ...
        if (yyvstack[yysp]) {
            let rv = checkActionBlock$1(yyvstack[yysp].action, yylstack[yysp]);
            if (rv) {
                ...
                `);
                    ...
                    let indentedSrc = rmCommonWS$2([yyvstack[yysp].action]).split('\n').join('\n    ');
    ...
    case 115:
        ...
        this.$ = {
            action: trimActionCode$1(yyvstack[yysp]),
            isArrowAction: false
        };
        break;
    ...
    case 116:
        ...
        {
        let src = trimActionCode$1(yyvstack[yysp], {
            dontTrimSurroundingCurlyBraces: true
        });
        src = braceArrowActionCode$1(src);
        this.$ = {
            action: `this.$ = ${src}`,
            isArrowAction: true
        };
    ...
    case 127:
        ...
        let rv = checkActionBlock$1(fileContent);
        if (rv) {
            yyparser.yyError(rmCommonWS$2`
    ...
    case 15:
        ...
        {
        let srcCode = trimActionCode$2(yyvstack[yysp - 1] + yyvstack[yysp], {
            startMarker: yyvstack[yysp - 2]
        });
        if (srcCode) {
            let rv = checkActionBlock$2(srcCode, yylstack[yysp - 1], yy);
            if (rv) {
                yyparser.yyError(rmCommonWS$3`
    ...
    case 25:
        ...
        let srcCode = trimActionCode$2(yyvstack[yysp - 2] + yyvstack[yysp - 1], {
            startMarker: yyvstack[yysp - 3]
        });
        let rv = checkActionBlock$2(srcCode, yylstack[yysp - 2], yy);
        if (rv) {
            yyparser.yyError(rmCommonWS$3`
    ...
    case 51:
        ...
        srcCode = 'return ' + braceArrowActionCode$2(srcCode);
        
        let rv = checkActionBlock$2(srcCode, yylstack[yysp - 1], yy);
        if (rv) {
            let indentedSrc = rmCommonWS$3([srcCode]).split('\n').join('\n    ');
    ...
    case 128:
        ...
        {
        let srcCode = trimActionCode$2(yyvstack[yysp]);
        if (srcCode) {
            let rv = checkActionBlock$2(srcCode, yylstack[yysp], yy);
            if (rv) {
                yyparser.yyError(rmCommonWS$3`
    ...
    case 136:
        ...
        `);
            ...
            let srcCode = trimActionCode$2(fileContent);
            if (srcCode) {
                let rv = checkActionBlock$2(srcCode, this._$, yy);
                if (rv) {
                    yyparser.yyError(rmCommonWS$3`
    ...
    // --------- END OF REPORT -----------
        ...
        rules: [
          ...
          /*  39: */  /^(?:(\r\n|\n|\r))/,
          /*  40: */  /^(?:([^\S\n\r])+)/,
          /*  41: */  new XRegExp__default['default']('^(?:([\\p{Alphabetic}_](?:[\\p{Alphabetic}\\p{Number}_])*))', ''),
          /*  42: */  /^(?:(\r\n|\n|\r)+)/,
          /*  43: */  /^(?:$)/,
          /*  44: */  /^(?:(\r\n|\n|\r)+)/,
          /*  45: */  /^(?:([^\S\n\r])+)/,
          ...
          /*  80: */  /^(?:%array\b)/,
          /*  81: */  /^(?:%include\b)/,
          /*  82: */  new XRegExp__default['default'](
            '^(?:%([\\p{Alphabetic}_](?:[\\p{Alphabetic}\\p{Number}\\-_]*(?:[\\p{Alphabetic}\\p{Number}_]))?)([^\\n\\r]*))',
            ''
          ),
          /*  83: */  /^(?:%%)/,
          ...
          /*  86: */  new XRegExp__default['default']('^(?:\\{([\\p{Alphabetic}_](?:[\\p{Alphabetic}\\p{Number}_])*)\\})', ''),
          /*  87: */  /^(?:\{)/,
          /*  88: */  /^(?:\})/,
          /*  89: */  /^(?:(?:\\[^\n\r]|[^\n\r\]{])+)/,
          /*  90: */  /^(?:\{)/,
