...
case 51:
    ...
    let ast = checkActionBlock(srcCode, yylstack[yysp - 1], yy.options);
    if (ast.fault) {
...
case 52:
    ...
    {
    let rv = yyparser.yyError(rmCommonWS`
...
case 69:
    ...
    if (yyvstack[yysp].fault) {
        ...
        let srcCode = trimActionCode(yyvstack[yysp].srcCode, {
            startMarker: yyvstack[yysp].action_start_marker
        });
    
        let ast = checkActionBlock(srcCode, yylstack[yysp], yy.options);
        this.$ = {
            ast, 
            fault: ast.fault,
            srcCode
...
case 137:
    ...
    if (srcCode) {
        let rv = checkActionBlock(srcCode, yylstack[yysp], yy.options);
        if (rv.fault) {
            yyparser.yyError(rmCommonWS`
...
case 145:
    ...
    if (!fs.existsSync(include_path)) {
        ...
        if (srcCode) {
            let rv = checkActionBlock(srcCode, this._$, yy.options);
            if (rv.fault) {
                yyparser.yyError(rmCommonWS`
